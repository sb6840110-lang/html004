...
<body class="bg-slate-100 font-sans min-h-screen pb-12">
    ...
    <main class="max-w-5xl mx-auto mt-6 px-4 space-y-6">
        ...
        <!-- Live Attendance Panel (จะแสดงขึ้นมาทุกครั้งเมื่อมีการเข้าสู่ระบบไม่ว่าจะเป็นผู้ใช้คนไหนก็ตาม) -->
    ...
    <script>
        ...
        function loginUser(role, displayName) {
            ...
            // โหลดและอัปเดตข้อมูลประวัติและ Dashboard ทันทีไม่ว่าจะเป็นใครเข้าสู่ระบบ
            loadStoredAttendanceData();
        ...
        function renderAttendanceTable() {
            ...
            tbody.innerHTML = attendanceData.map(item => {
                ...
                if (item.status === 'ปกติ') {
                    ...
                    statusBadge = '<span class="bg-rose-100 text-rose-800 text-xs font-bold px-2.5 py-1 rounded-lg">ไม่เข้าร่วม</span>';
                }
                ...
                return `
                    <tr class="hover:bg-slate-50 transition">
                        ...
                        <td class="p-3">
                            <img src="${item.imgUrl}" class="w-10 h-10 rounded-lg object-cover border border-slate-200 shadow-sm">
                        </td>
        ...
        function updateSummaryCards() {
            const normalCount = attendanceData.filter(i => i.status === 'ปกติ').length;
            const lateCount = attendanceData.filter(i => i.status === 'สาย').length;
            const absentCount = attendanceData.filter(i => i.status === 'ไม่เข้าร่วมกิจกรรม').length;
